...
import com.seeyuan.logistics.adapter.StartSelectCarAdapter;
...
public class SelectCarActivity extends BaseActivity implements
		OnClickListener, OnDataReceiveListener,OnItemClickListener {
	/**
	 * 返回按钮
	 */
	...
	private StartSelectCarAdapter mAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_car_select); // 软件activity的布局
	...
	private void doSecendBack() {
		if (SelectCarActivity.this.isFinishing()) {
			return;
		}
	...
	public void initView() {
		...
		defaulttitle_title_tv.setText("选择车辆定位");

		if ( !isLogin ) {
			findViewById(R.id.car_manager_new).setVisibility(View.GONE);
		}
	...
	private void initAdapter() {
		...
		mAdapter = new StartSelectCarAdapter(context, myHandler, mDataList);
	...
	protected void doSearchGoods(Message msg) {
		Intent intent = new Intent(SelectCarActivity.this,
...
//		selectedList = mAdapter.getSelectedList();
	...
	private void doGetCarSourceSuccss(Object data) {
		...
		try {
			...
			mListView.setAdapter(mAdapter);
			mListView.setOnItemClickListener(SelectCarActivity.this);
			// mAdapter.notifyDataSetChanged();
			car_manager_refresh_view.onRefreshComplete();
		} catch (Exception e) {
			ToastUtil.show(context, "获取车辆信息失败，请重新获取");
...
//		intent.putExtra("carsDto", mDataList.get(position));
			sPreferences.edit().putString("carsNum", mDataList.get(position - 1).getVehicleNum()).commit();
			intent.putExtra("myTag", 1);
			startActivity(intent);
			finish();
		} else {
			Intent intent = new Intent();
			intent.putExtra("carNum", mDataList.get(position -1).getVehicleNum());
