...
//		pay_detail_type.setOnClickListener(this);
		xianxia_layout = (LinearLayout) findViewById(R.id.xianxia_layout);
		RadioButton radio_ali = (RadioButton) findViewById(R.id.radio_ali);
		radio_ali.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				// TODO Auto-generated method stub
				if (isChecked) {
//					xianxia_layout.setVisibility(View.GONE);
					little(xianxia_layout);
				} else {
//					xianxia_layout.setVisibility(View.VISIBLE);
					big(xianxia_layout);
				}
				
			}
		});
		RadioButton radio_xianxia = (RadioButton) findViewById(R.id.radio_xianxia);
		radio_xianxia.setOnCheckedChangeListener(new OnCheckedChangeListener() {
...
//					xianxia_layout.setVisibility(View.GONE);
	 ...
	 * @param layout
	 */
	private void fadeOut(final LinearLayout layout){
		ObjectAnimator animator = ObjectAnimator.ofFloat(layout, "translationX", 0f, screenWidth);
	 ...
	 * @param layout
	 */
	private void fadeIn(final LinearLayout layout){
		ObjectAnimator animator = ObjectAnimator.ofFloat(layout, "translationX", screenWidth, 0f);
		animator.setDuration(1500);
		animator.addListener(new AnimatorListenerAdapter() {
			@Override
		...
		public void handleMessage(Message msg) {
			...
			case SDK_PAY_FLAG: {
				...
				if (TextUtils.equals(resultStatus, "9000")) {
					Toast.makeText(PaymentDetailActivity.this, "支付成功", Toast.LENGTH_SHORT).show();
					...
					if (TextUtils.equals(resultStatus, "8000")) {
						Toast.makeText(PaymentDetailActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();

					} else {
						// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
						Log.i("支付结果", resultStatus + "," + resultInfo);
						Toast.makeText(PaymentDetailActivity.this, "支付失败", Toast.LENGTH_SHORT).show();
			...
			case SDK_CHECK_FLAG: {
				Toast.makeText(PaymentDetailActivity.this, "检查结果为：" + msg.obj, Toast.LENGTH_SHORT).show();
				break;
			}
			default:
				break;
	 ...
	 * 线下支付布局
	 */
	private LinearLayout xianxia_layout;
	...
	public void pay() {
		...
		Runnable payRunnable = new Runnable() {
			...
			public void run() {
				// 构造PayTask 对象
				PayTask alipay = new PayTask(PaymentDetailActivity.this);
				// 调用支付接口，获取支付结果
