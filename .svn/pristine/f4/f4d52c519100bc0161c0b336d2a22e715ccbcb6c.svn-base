 ...
 * 
 */
public class SearchFriendsMapActivity extends Activity implements
		OnClickListener {
	...
	private List<AroundFriends> mFriendsList = new ArrayList<AroundFriends>();
	...
	private void initView() {
		...
		defaulttitle_title_tv.setText(R.string.tab_around_friends);
		...
		infoAdapter = new PoiInfoAdapter(SearchFriendsMapActivity.this,
				poiInfoResults);
	...
	private void initMap() {
		...
		// 定位初始化
		mLocClient = new LocationClient(SearchFriendsMapActivity.this);
	...
	private void initOverlay() {
		...
		mFriendsList = getIntent().getParcelableArrayListExtra(
				"FirendPositionInfo");
		int friendsSize = mFriendsList.size();
		for (int i = 0; i < friendsSize; i++) {
			double lat = Double.parseDouble(mFriendsList.get(i).getLatitude());
			double lon = Double.parseDouble(mFriendsList.get(i).getLongitude());
			GeoPoint point = new GeoPoint((int) (lat * 1E6), (int) (lon * 1E6));
			pointsList.add(point);
			OverlayItem item = new OverlayItem(point, mFriendsList.get(i)
					.getUserName(), "");
	...
	public void onClick(View v) {
		...
		case R.id.searchbtn:
			...
			if (!TextUtils.isEmpty(key)) {
				...
				Toast.makeText(SearchFriendsMapActivity.this, "请填写地址",
						Toast.LENGTH_SHORT).show();
			}
			break;
		case R.id.maintitle_back_iv:
			CommonUtils.finishActivity(SearchFriendsMapActivity.this);
			break;
		}
	}
	...
	class MySearchListener implements MKSearchListener {
		...
		public void onGetPoiResult(MKPoiResult result, int type, int error) {
			...
			if (error != 0 || result == null) {
				Toast.makeText(SearchFriendsMapActivity.this, "抱歉，未找到结果",
