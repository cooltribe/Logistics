 ...
 * 
 */
public class ParkingLotActivity extends BaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_parking_lot); // 软件activity的布局
	...
	public void initView() {
		...
		defaulttitle_title_tv.setText(R.string.parking_lot_hint);
		...
		mMapView = (MapView) findViewById(R.id.map_parking_lot);
	...
	private void initMap() {
		...
		// 定位初始化
		mLocClient = new LocationClient(ParkingLotActivity.this);
	...
	public void onClick(View view) {
		...
		case R.id.maintitle_back_iv:
			CommonUtils.finishActivity(ParkingLotActivity.this);
			break;
	...
	private void doSearchParkingLot() {
		if (TextUtils.isEmpty(currentCity)) {
			ToastUtil.show(context, "地图正在加载,请稍后再试");
		} else {
			// mSearch.poiSearchInCity(currentCity, "停车场");
			mSearch.poiSearchNearBy("停车场", currentPoint, 3000);
		}
	}
	...
	class MySearchListener implements MKSearchListener {
		...
		public void onGetPoiResult(MKPoiResult result, int type, int error) {
			...
			if (error != 0 || result == null) {
				Toast.makeText(ParkingLotActivity.this, "抱歉，未找到结果",
